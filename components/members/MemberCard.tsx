...
export default function MemberCard({ member, onPrint }: MemberCardProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(cardElement, {
        ...
        useCORS: true, // Allow external images from Supabase or Unsplash
        backgroundColor: null,
      });

      const imgData = canvas.toDataURL("image/jpeg", 1.0);

      // Create PDF perfectly matched to the card's dimensions
      const pdf = new jsPDF({
  ...
  return (
    <div className="space-y-4">
      {/* Badge Printable Container */}
      <div 
        id={`member-card-${member.id}`}
        className="w-[450px] mx-auto bg-gradient-to-br from-inverse-surface via-primary to-inverse-surface text-white rounded-3xl p-6 shadow-2xl border-2 border-secondary/40 relative overflow-hidden text-left"
        // Force a specific width so the PDF generation is consistent across all screen sizes
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/20 relative z-10">
          ...
          <span className="px-2.5 py-1 bg-secondary text-white text-[9px] font-extrabold uppercase rounded-full tracking-wider shadow-sm flex items-center gap-1">
        ...
        <div className="py-5 grid grid-cols-3 gap-4 items-center relative z-10">
          ...
          <div className="col-span-1 text-center space-y-1.5">
            <div className="relative inline-block">
              <img
                ...
                className="w-24 h-28 object-cover rounded-2xl border-2 border-secondary shadow-lg mx-auto"
                crossOrigin="anonymous" // Important for html2canvas
              />
              <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-emerald-600 text-[8px] font-bold text-white rounded-full uppercase tracking-wider shadow whitespace-nowrap">
                {member.status}
              </span>
            </div>
          </div>
          ...
          <div className="col-span-2 space-y-2 text-xs">
            <div>
              <span className="text-[10px] text-secondary-fixed font-mono uppercase tracking-wider block font-semibold">
                Matricule Membre
              </span>
              <span className="font-mono font-extrabold text-sm text-white bg-white/10 px-2.5 py-1 rounded-xl inline-block border border-white/15">
                {member.memberNumber}
              </span>
            </div>
        ...
        <div className="pt-3 border-t border-white/15 flex items-center justify-between relative z-10 text-[10px]">
          ...
          <div className="bg-white p-1.5 rounded-xl shadow-md text-primary flex items-center gap-1">
